...
//    var_dump($months_between );
    ...
    $price_info = $this->arrival_month_price($this->arrival_date, $this->departure_date, $room_for_check);
   
//    var_dump($this->arrival_month_price($this->arrival_date, $this->departure_date, $room_for_check));
    $price += $this->arrival_month_price($this->arrival_date, $this->departure_date, $room_for_check)['final_price'];
    $price_no_discount += $this->arrival_month_price($this->arrival_date, $this->departure_date, $room_for_check)['price_no_discount'];
    $total_discount += $this->arrival_month_price($this->arrival_date, $this->departure_date, $room_for_check)['total_discount'];
//    echo "<h1>$price</h1>";
//     var_dump($price_no_discount);
    ...
    if ($arrival_month !== $departure_month){
          ...
          $price_info = $this->other_months_prices($this->arrival_date, $this->departure_date, $room_for_check);
        
        
          $price += $price_info['final_price'];
          $price_no_discount += $price_info['price_no_discount'];
          $total_discount += $price_info['total_discount'];
//          echo "<h1>cijena u ostalim...$price</h1>";
   } 
        $price_info = array();
        $price_info['price'] = $price;
        $price_info['price_no_discount'] = $price_no_discount;
        $price_info['total_discount'] = $total_discount;
...
//    ----CIJENA U POCETNOM MJESECU-------------------
    private function arrival_month_price(DateTime $date1,DateTime $date2, Room $room_for_check){
      ...
      $days_in_month = ($arrival_month == $departure_month)? $departure_day : self::no_of_days_in_month($arrival_month, $year);
               ...
               $total_discount += $this->$month_discounts[$i]/100*$this->$month_prices[$i];
               $price_no_discont += $this->$month_prices[$i];
        
                 $price += $this->$month_prices[$i] - $this->$month_discounts[$i]/100*$this->$month_prices[$i];
//                   VAZNA PROVJERA SLIJEDI!!!
//                        echo "<h4>first month " . $i . " : ".$this->$month_prices[$i] . "e - " .  $this->$month_discounts[$i]. "%</h4><br>";  
//                        echo "price: " . $price . "<br>"; 
//                        echo "no_disc: " . $price_no_discont . "<br>"; 
//                        echo "disc: " . $total_discount . "<br>"; 
                 }
        $price_info = array();
        $price_info["price_no_discount"] = $price_no_discont;
        $price_info["total_discount"] = $total_discount;
        $price_info["final_price"] = $price;
        
//        echo "<h2>Cijena u prvom: $price</h2>";
//        echo "<h3>Popust u prvom: $total_discount</h3>";
//        echo "<h4>bez popusta u prvom: $price_no_discont</h4>";
...
//        echo "<h2>Cijena u ostalim: $price</h2>";
        return $price_info;
...
//                
//                
                echo "<div class='cal_date ".$ava_class."' data-price='".$this->$month_prices[$i]."' data-discount='".$this->$month_discounts[$i]."' data-ym='$year-$month'> $i </div>";
               }
